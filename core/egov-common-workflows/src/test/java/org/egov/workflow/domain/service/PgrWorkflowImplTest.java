...
public class PgrWorkflowImplTest {
	...
	@MockBean
	private Workflow pgrWorkflowImpl;
	...
	private Position getPosition() {
		return Position.builder().id(2L).name("Accounts Officer").build();
	}

	private User getUserInfo() {
		return User.builder().id(16L).build();
	}
	...
	private ProcessInstanceRequest getProcessInstanceRequest() {
		...
		final ProcessInstance processInstance = ProcessInstance.builder()
				.senderName("narasappa").status("Registered").details("Complaint is registered")
				.createdDate(new Date()).tenantId("tenantId").attributes(valuesMap).build();
		return ProcessInstanceRequest.builder().requestInfo(requestInfo).processInstance(processInstance).build();
	...
	public void testForStartWorkflowWhenAssigneeIdNull() {
		...
		when(complaintRouterService.getAssignee(1L, "C001", null, 1L,"tenantId",new RequestInfo())).thenReturn(position);
		when(positionRepository.getById(2L, processInstanceRequest.getRequestInfo())).thenReturn(position);
		when(stateService.create(new State())).thenReturn(stateExpected);

		final ProcessInstanceResponse actualResponse = pgrWorkflowImpl.start(processInstanceRequest);

		assertEquals(stateExpected.getOwnerPosition(), actualResponse.getProcessInstance().getAssignee().getId());
	}
	...
	public void testForEndWorkflow() throws Exception {
        ...
        final UserResponse expectedUserRequest = UserResponse.builder().responseInfo(new ResponseInfo())
                .users(Collections.singletonList(user)).build();

        final ProcessInstance expectedProcessInstance = ProcessInstance.builder()
                .type("Complaint").details("Workflow Terminated").assignee(Position.builder().id(2L).build())
                .attributes(new HashMap<String, Attribute>()).businessKey("765").build();
        ...
        pgrWorkflowImpl.end(ProcessInstanceRequest.builder().requestInfo(requestInfo).processInstance(expectedProcessInstance).build());
	}
	...
	public void testForGetWorkflowHistoryByWorkFlowId() {
		...
		//when(employeeRepository.getEmployeeForPositionAndTenantId(3l,"tenantId")).thenReturn(employeeRes);
...
/*	@Test
	public void testForUpdateWorkflowForEscalation(){
        ...
        State state = prepareState();
        Position expectedPosition = Position.builder().id(3L).name("kiran").build();
        when(stateService.update(new State())).thenReturn(state);
        when(stateService.getStateByIdAndTenantId(2l, "ap.public")).thenReturn(state);
        when(complaintRouterService.getAssignee(null,"",4L,1L,"ap.public",new RequestInfo())).thenReturn(expectedPosition);
	...
	private UserResponse getUserResponse() {
		...
		return UserResponse.builder().responseInfo(new ResponseInfo()).users(Collections.singletonList(user)).build();
	}
	...
	private Task getTask() {
		...
		Task task = Task.builder().attributes(valuesMap).assignee(Position.builder().id(3L).build()).id("2").senderName("narasappa").status("PROCESSING")
	...
	private Task getTaskForEscalation(){
        ...
        final Attribute attributePreviousAssignee = new Attribute(false,"","",true,"",value4);
        ...
        Task task = Task.builder().attributes(valuesMap).assignee(null).id("2").senderName("narasappa").status("PROCESSING")
            .createdDate(new Date()).build();
